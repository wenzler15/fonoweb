...
import { ChevronLeft, ChevronRight, WindowSharp } from '@mui/icons-material'
...
export function Availability() {
  ...
  const getPatients = async () => {
    ...
    getAppointments(pacientsArray);
  }

  const getAppointments = async (patientsInfo) => {
    const userString = localStorage.getItem('@fonoweb/auth');
    const userJson = JSON.parse(userString)
    ...
    resp.data.map((item) => {
      const begin = new Date(item.when);
      const finish = new Date(item.end);
  ...
  const updateCalendar = async (event) => {
    const userString = localStorage.getItem('@fonoweb/auth');
    const userJson = JSON.parse(userString)
    ...
    if (event.event_id) {
      await api.patch(`/appointments/${event.event_id}`, toSend, {
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${userJson.state.token}`
        }
      ...
      await api.post(`/appointments/create`, toSend, {
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${userJson.state.token}`
        }
